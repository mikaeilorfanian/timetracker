...
class TestSearchUserActivities:
    ...
    def test_correct_user_activities_are_found_when_user_has_multiple_activities(
            self, test_user, test_sleeping_activity, test_working_activity, test_db):
    ...
    def test_no_user_activities_are_found_when_user_has_no_activities(self, test_user):
        activities = ActivitySearch.user_activities(test_user)
...
class TestSearchForUserActivitiesWithSpecificCategoryStartedToday:
    ...
    def test_one_activity_returned_when_only_one_started_today(self, test_db, test_activity, test_user):
        todays_activities = ActivitySearch.user_activities_today_in_this_category(test_user, test_activity.category)
        assert len(todays_activities) == 1
        assert test_activity in todays_activities
    ...
    def test_user_has_more_than_one_activity_today_but_only_one_is_what_we_want_and_more_activities_on_other_days(self, test_user_with_multiple_activities_on_multiple_days, test_db, test_activity):
        ...
        todays_activities = ActivitySearch.user_activities_today_in_this_category(test_user, 'working')
        assert len(todays_activities) == 1
        assert test_working_activity in todays_activities
...
class TestSearchForActivity:
    ...
    def test_correct_activity_is_found(self, test_user, test_activity, test_db):
        activity = ActivitySearch.fetch_from_db(test_user, test_activity)
        assert activity._id == test_activity._id

    def test_exception_thrown_when_no_activity_found_in_db_with_that_id(self, test_user, test_db):
        a = ActivityManager.start_new_activity(test_user, 'test_activity')
        with pytest.raises(RecordNotFoundError):
            ActivitySearch.fetch_from_db(test_user, a)
